...
**HandleHQ — AI Customer Support Automation.** A deployed, multi-tenant SaaS that turns
unstructured inbound messages (email and WhatsApp) into structured, auto-triaged support
conversations through AI-guided intake. Flask, PostgreSQL, OpenAI API, with CI, branch
protection, and a real production deployment.

The problem it solves: most small businesses handle customer inquiries manually with no
structure or tracking. HandleHQ slots into that workflow and automates the intake layer.
...
| **Tools** | Git, Postman, Docker, nginx, DigitalOcean, CI/CD |
...
> _Small, focused repos extracted from real production code. Pinned as they go live._

- **openai-retry** — a production-grade retry/backoff wrapper for the OpenAI SDK, with
  typed quota-exhaustion handling and jittered exponential backoff. _(coming soon)_
- More integration and automation demos on the way.
